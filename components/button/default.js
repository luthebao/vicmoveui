...
const ButtonDefault = ({ children, buttonStyle, shapes, type, size, className, callback }) => {
    ...
    return (
        <button
            type={type}
            className={classes + ' uppercase btn text-white hover:opacity-90 ' + (className || '')} onClick={callback}>
            {children}
        </button>
    );
}
