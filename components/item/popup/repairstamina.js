...
const RepairStamina = ({ onClose, itemid, session }) => {

    const [processing, setProcessing] = useState(false)

    const handleRepair = async () => {
        setProcessing(true)
        const fet_rp = await fetch("/api/user/item/rp-stamina", {
            headers: {
    ...
    return (
        <div className="flex flex-col text-center">
            <p className="py-5">Are you sure to repair 30 Stamina with 20 VIM ?</p>
